...
const Header = () => {
  ...
  const handleToggleMenuMobile = () => {
    setIsMenuMobile((prev) => !prev);
  };

  const hItems = [
    { id: "home", title: "Home", url: `/#home` },
    { id: "about", title: "About", url: `/#about` },
    { id: "work", title: "Work History", url: `/#work` },
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-[11]  shadow-sm shadow-neutral-500 py-2.5 ${
          isBlogPage ? "bg-white text-black" : "bg-background text-neutral-200"
        }`}
      >
        <div className="container flex justify-between items-center h-full">
          ...
          >
            {isMenuMobile ? <AlignJustify /> : <SquareX />}
          </div>
        </div>

        <div
          className={`fixed ${
            isMenuMobile ? "h-screen w-full pt-1" : "invisible h-0 w-full pt-0"
          }  md:hidden bg-[#13161c] transition-all duration-500 ease-in-out `}
        >
          <div className="container">
            <ul
              className={`${
                isMenuMobile ? "visible" : "invisible"
